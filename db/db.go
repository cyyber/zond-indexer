...
func saveValidators(data *types.EpochData, tx *sqlx.Tx, client rpc.Client) error {
	...
	if data.Epoch == 0 {
		var err error
		genesisBalances, err = MongodbClient.GetValidatorBalanceHistory([]uint64{}, 0, 0)
		if err != nil {
			return err
		}
	}
	...
	for _, newValidator := range newValidators {
		...
		if newValidator.ActivationEpoch == 0 {
			...
			balance, err = MongodbClient.GetValidatorBalanceHistory([]uint64{newValidator.Validatorindex}, newValidator.ActivationEpoch, newValidator.ActivationEpoch)
			if err != nil {
				return err
			}
		}
