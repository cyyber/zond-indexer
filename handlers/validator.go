...
func Validator(c *gin.Context) {
	...
	if err != nil {
		logger.Errorf("error retrieving flashes for validator %v: %v", c.Param("index"), err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}

	// Request came with a hash
	if strings.Contains(c.Param("index"), "0x") || len(c.Param("index")) == 96 {
		pubKey, err := hex.DecodeString(strings.Replace(c.Param("index"), "0x", "", -1))
		if err != nil {
	...
	g.Go(func() error {
		...
		if len(proposedToday) > 0 {
			// get el data
			execBlocks, err := db.MongodbClient.GetBlocksIndexedMultiple(proposedToday, 10000)
			if err != nil {
				return fmt.Errorf("error retrieving execution blocks data from mongodb: %v", err)
			}
	...
	g.Go(func() error {
		if validatorPageData.AttestationsCount > 0 {
			...
			if lookback > 0 {
				...
				attestationsNotInStats, err := db.MongodbClient.GetValidatorAttestationHistory([]uint64{index}, finalizedEpoch-uint64(lookback), finalizedEpoch)
				if err != nil {
	...
	g.Go(func() error {
		eff, err := db.MongodbClient.GetValidatorEffectiveness([]uint64{index}, validatorPageData.Epoch-1)
		if err != nil {
	...
	g.Go(func() error {
		...
		if len(actualSyncPeriods) > 0 {
			...
			if actualSyncPeriods[0].LastEpoch > lastExportedEpoch {
				...
				res, err := db.MongodbClient.GetValidatorSyncDutiesHistory([]uint64{index}, latestEpoch-uint64(lookback), latestEpoch)
				if err != nil {
					return fmt.Errorf("error retrieving validator sync participations data from mongodb: %v", err)
				}
...
func ValidatorProposedBlocks(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
...
func ValidatorAttestations(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
	...
	if totalCount > 0 {
		attestationData, err := db.MongodbClient.GetValidatorAttestationHistory([]uint64{index}, uint64(int64(lastAttestationEpoch)-start)-uint64(length), uint64(int64(lastAttestationEpoch)-start))
		if err != nil {
...
func ValidatorWithdrawals(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
...
func ValidatorSync(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
	...
	if totalCount > 0 && start <= totalCount {
		...
		syncDuties, err := db.MongodbClient.GetValidatorSyncDutiesHistoryOrdered(validatorIndex, firstShownEpoch-limit, firstShownEpoch, ascOrdering)
		if err != nil {
		...
		if nextPeriodLimit != 0 {
			nextPeriodSyncDuties, err := db.MongodbClient.GetValidatorSyncDutiesHistoryOrdered(validatorIndex, lastShownEpoch-uint64(nextPeriodLimit), lastShownEpoch, ascOrdering)
			if err != nil {
...
func ValidatorHistory(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
	currency := GetCurrency(r)
	...
	var incomeDetails map[uint64]map[uint64]*types.ValidatorEpochIncome
	g.Go(func() error {
		var err error
		incomeDetails, err = db.MongodbClient.GetValidatorIncomeDetailsHistory([]uint64{index}, startEpoch, endEpoch)
		if err != nil {
...
func ValidatorDeposits(c *gin.Context) {
	w := c.Writer
	w.Header().Set("Content-Type", "application/json")

	pubkey, err := hex.DecodeString(strings.Replace(c.Param("index"), "0x", "", -1))
	if err != nil {
		logger.Errorf("error parsing validator public key %v: %v", c.Param("index"), err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
	...
	deposits, err := db.GetValidatorDeposits(pubkey)
	if err != nil {
		logger.Errorf("error getting validator-deposits for %v: %v", c.Param("index"), err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		logger.Errorf("error encoding validator-deposits for %v: %v", c.Param("index"), err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
}
...
func ValidatorSlashings(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
...
func ValidatorAttestationInclusionEffectiveness(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
	...
	eff, err := db.MongodbClient.GetValidatorEffectiveness([]uint64{index}, services.LatestEpoch()-1)
	if err != nil {
...
func sanitizeMessage(msg string) ([]byte, error) {
	subString := "zondscan.in"

	if strings.Contains(msg, subString) {
		return []byte(msg), nil
	} else {
		decoded := strings.Replace(msg, "Q", "", -1)
		...
		return nil, errors.New("zondscan.in was not found")
...
func ValidatorSave(c *gin.Context) {
	w := c.Writer
	r := c.Request
	pubkey := r.FormValue("index")
...
// Validator returns validator data using a go template
func ValidatorStatsTable(c *gin.Context) {
	w := c.Writer
	r := c.Request
