...
func SlotDepositData(c *gin.Context) {
	w := c.Writer
	r := c.Request
	...
	if err != nil || len(slotOrHash) != 64 {
		blockSlot, err = strconv.ParseInt(c.Param("slot"), 10, 64)
		if err != nil {
			logger.Errorf("error parsing slotOrHash url parameter %v, err: %v", c.Param("slot"), err)
...
func SlotVoteData(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
...
func BlockTransactionsData(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")

	slot, err := strconv.ParseUint(c.Param("block"), 10, 64)
	if err != nil {
		logger.Errorf("error parsing slot url parameter %v, err: %v", c.Param("slot"), err)
		http.Error(w, "Internal server error", http.StatusInternalServerError)
		return
	}
...
func SlotAttestationsData(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
...
func SlotWithdrawalData(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
...
func SlotBlsChangeData(c *gin.Context) {
	w := c.Writer
	r := c.Request
	w.Header().Set("Content-Type", "application/json")
