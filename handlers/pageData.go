...
func InitPageData(w http.ResponseWriter, r *http.Request, active, path, title string, mainTemplates []string) *types.PageData {
	...
	if title == "" {
		fullTitle = fmt.Sprintf("%v - zondscan.in - %v", utils.Config.Frontend.SiteName, time.Now().Year())
	}
	...
	data := &types.PageData{
		Meta: &types.Meta{
			Title:       fullTitle,
			Description: "zondscan.in makes Zond accessible to non-technical end users",
			Path:        path,
...
func SetPageDataTitle(pageData *types.PageData, title string) {
	if title == "" {
		pageData.Meta.Title = fmt.Sprintf("%v - zondscan.in - %v", utils.Config.Frontend.SiteName, time.Now().Year())
	} else {
		pageData.Meta.Title = fmt.Sprintf("%v - %v - zondscan.in - %v", title, utils.Config.Frontend.SiteName, time.Now().Year())
	}
}
