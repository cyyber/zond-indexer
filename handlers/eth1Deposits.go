...
func Eth1DepositsData(c *gin.Context) {
	w := c.Writer
	r := c.Request
...
func Eth1DepositsLeaderboard(c *gin.Context) {
	w := c.Writer
	r := c.Request
...
// Eth1DepositsData will return eth1-deposits as json
func Eth1DepositsLeaderboardData(c *gin.Context) {
	w := c.Writer
	r := c.Request
	currency := GetCurrency(r)
	w.Header().Set("Content-Type", "application/json")
